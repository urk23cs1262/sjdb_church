/**
 * Bible Reference Helper
 * Translates between English and Tamil Bible book references (Catholic CCBI / திருவிவிலியம் standards)
 */
...
/**
 * Normalizes input text for book comparison
 */
...
/**
 * Parse reference into book part and chapter:verse numbers
 */
function parseRef(rawRef) {
  if (!rawRef) return { bookPart: '', numPart: '' };
  let clean = String(rawRef).trim();

  // If reference has a bilingual slash e.g. "சங்கீதம் / Psalm 23:1"
  if (clean.includes('/')) {
    const parts = clean.split('/');
    // Check if one has Tamil
  ...
  // Match numbers at the end like "3:15", "23:1", "13:4-8", "5:1-12", "1-5"
...
/**
 * Find matching Bible book entry
 */
...
/**
 * Returns Tamil Bible reference (e.g. "1 பேதுரு 3:15")
 */
function getTamilBibleReference(rawRef) {
  if (!rawRef) return '';
...
/**
 * Returns English Bible reference (e.g. "1 Peter 3:15")
 */
function getEnglishBibleReference(rawRef) {
  if (!rawRef) return '';
  const parsed = parseRef(rawRef);
 ...
 */
function getBilingualBibleReference(rawRef) {
  return {
...
module.exports = {
  getTamilBibleReference,
  getEnglishBibleReference,
  getBilingualBibleReference,
  BIBLE_BOOKS
};
