...
export default function MaintenanceAdmin() {
  ...
  const [actionLoading, setActionLoading] = useState(null); // 'enabling' | 'disabling' | 'emergency' | 'saving_display' | 'saving_scheduler' | 'showcase_banner' | 'stop_banner' | null
  const [saveFeedback, setSaveFeedback] = useState(null); // 'display_saved' | 'schedule_saved' | null
  ...
  const fetchSettings = async (silent = false) => {
    try {
      ...
      if (!silent) toast.error('Failed to load maintenance settings');
    } finally {
  ...
  // Background non-blocking notification polling while a transition is pending
  useEffect(() => {
    if (activeEvent && !activeEvent.notificationSent) {
      ...
      pollIntervalRef.current = setInterval(async () => {
        try {
          ...
          if (res.data.success) {
            ...
            if (latest && latest._id === activeEvent._id) {
              setActiveEvent(latest);
              if (latest.notificationSent) {
  ...
  const handleSaveDisplaySettings = async () => {
    setActionLoading('saving_display');
    try {
      const res = await api.put('/maintenance/settings', settings);
      if (res.data.success) {
        setSettings(res.data.settings);
        setSaveFeedback('display_saved');
  ...
  const handleSaveSchedulerSettings = async () => {
    setActionLoading('saving_scheduler');
    try {
      const res = await api.put('/maintenance/settings', settings);
      if (res.data.success) {
        setSettings(res.data.settings);
        setSaveFeedback('schedule_saved');
        toast.success(settings.scheduler?.isEnabled ? 'Maintenance Schedule is Active!' : 'Schedule settings saved successfully');
        setTimeout(() => setSaveFeedback(null), 3500);
      }
    } catch (err) {
  ...
  // Pre-Maintenance Notice Banner Showcase
  const handleShowcaseBanner = async () => {
    setActionLoading('showcase_banner');
    try {
      const payload = {
  ...
  // 1. Enable Maintenance Mode Action
  const handleEnableMaintenance = async () => {
    setActionLoading('enabling');
    setEnableModalOpen(false);
    try {
      const res = await api.post('/maintenance/toggle', {
        isEnabled: true,
        reason: customReason || settings?.message || 'Scheduled System Maintenance',
        category: settings?.category || 'Scheduled Update'
      });

      if (res.data.success) {
        setSettings(res.data.settings);
        if (res.data.activeEvent) setActiveEvent(res.data.activeEvent);
        toast.success(res.data.message || 'Maintenance Mode Enabled');
        fetchSettings(true);
      }
    } catch (err) {
      toast.error('Failed to enable maintenance mode: ' + (err.response?.data?.message || err.message));
    } finally {
      setActionLoading(null);
    }
  };

  // 2. Disable Maintenance Mode Action
  const handleDisableMaintenance = async () => {
    setActionLoading('disabling');
    try {
      const res = await api.post('/maintenance/toggle', {
        isEnabled: false,
        reason: 'Maintenance Completed — System Restored to Live'
      });

      if (res.data.success) {
        setSettings(res.data.settings);
        if (res.data.activeEvent) setActiveEvent(res.data.activeEvent);
        toast.success('Maintenance Mode Disabled — Website is Live!');
        fetchSettings(true);
      }
    } catch (err) {
      toast.error('Failed to disable maintenance mode: ' + (err.response?.data?.message || err.message));
    } finally {
      setActionLoading(null);
    }
  };

  // 3. Emergency Shutdown Action
  const handleEmergencyShutdown = async () => {
    setActionLoading('emergency');
    try {
      const res = await api.post('/maintenance/emergency', {
        reason: emergencyReason || 'Emergency System Lockdown',
      ...
      if (res.data.success) {
        ...
        toast.error('🚨 EMERGENCY SHUTDOWN ACTIVATED! Website is locked.');
        fetchSettings(true);
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-3">
        <FiRefreshCw className="animate-spin text-4xl text-church-royal-blue" />
        <span className="text-sm font-semibold text-gray-500">Loading Operational Control Panel...</span>
      </div>
    );
  }

  const isLive = !settings?.isEnabled && !settings?.isEmergency && settings?.status !== 'maintenance' && settings?.status !== 'emergency';
  const isEmergency = Boolean(settings?.isEmergency) || settings?.status === 'emergency';
  const isMaintenance = !isLive && !isEmergency;
    ...
    <div className="w-full p-4 sm:p-6 lg:p-8 space-y-6 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-xl sm:text-2xl font-bold text-church-royal-blue flex items-center gap-2">
            <FiTool className="text-church-gold" /> Maintenance & Emergency Mode
          </h1>
          <p className="text-gray-500 text-xs sm:text-sm mt-0.5">
            Operational control panel for website access, WhatsApp bot status & emergency shutdowns
          </p>
        </div>

        {/* Primary Action Buttons with Immediate Loading Feedback */}
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            onClick={() => setPreviewOpen(true)}
            className="flex items-center gap-1.5 px-3.5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-xs font-bold shadow-md transition-all active:scale-95 cursor-pointer"
          >
          ...
          {isLive ? (
            <button
              onClick={() => {
                setCustomReason(settings?.message || 'Scheduled System Maintenance');
                setEnableModalOpen(true);
              }}
              disabled={actionLoading !== null}
              className="flex items-center gap-2 px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs sm:text-sm font-extrabold shadow-md transition-all active:scale-95 cursor-pointer disabled:opacity-50"
            >
              {actionLoading === 'enabling' ? (
                <>
                  <FiRefreshCw className="animate-spin text-sm" />
                  <span>Enabling Maintenance Mode...</span>
                </>
              ) : (
                <>
            ...
            <button
              ...
              disabled={actionLoading !== null}
              className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs sm:text-sm font-extrabold shadow-md transition-all active:scale-95 cursor-pointer disabled:opacity-50"
            >
              {actionLoading === 'disabling' ? (
                <>
                  <FiRefreshCw className="animate-spin text-sm" />
                  <span>Disabling Maintenance Mode...</span>
                </>
              ) : (
                <>
          ...
          <button
            ...
            disabled={actionLoading !== null}
            className="flex items-center gap-1.5 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs sm:text-sm font-extrabold shadow-md transition-all active:scale-95 cursor-pointer disabled:opacity-50"
          >
            {actionLoading === 'emergency' ? (
              <>
                <FiRefreshCw className="animate-spin text-sm" />
                <span>Activating Emergency Shutdown...</span>
              </>
            ) : (
              <>
      ...
      <div className={`p-4 sm:p-5 rounded-2xl border flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs transition-colors duration-300 ${
        ...
        <div className="flex items-center gap-2.5 text-xs font-bold flex-shrink-0">
          <span className="px-3 py-1.5 rounded-xl bg-white/90 border border-gray-200 shadow-2xs">
          ...
          <div className="flex items-center justify-between border-b pb-2.5">
            ...
            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold flex items-center gap-1.5 ${
              ...
              {activeEvent.notificationSent ? (
                ...
                <>
                  <FiRefreshCw className="text-xs animate-spin" /> Dispatching in Background...
                </>
              )}
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-blue-50/80 border border-blue-200 p-4 rounded-2xl flex items-center gap-3 text-xs text-blue-900 font-medium">
              ...
              <span>
                <strong>Operational Control:</strong> Changing system state takes effect <strong>immediately</strong> across the website and WhatsApp bot. Notification dispatch runs asynchronously in the background.
              </span>
            </div>
            ...
            {/* Assistance Contact & Media Header */}
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-5">
              <h2 className="text-base font-bold text-gray-900 flex items-center gap-2 border-b pb-3">
              ...
              <div className="pt-2 flex items-center gap-3">
                <button
                  ...
                  disabled={actionLoading !== null}
                  className="px-6 py-2.5 bg-church-gold hover:bg-amber-600 text-white font-bold text-xs sm:text-sm rounded-xl cursor-pointer disabled:opacity-50 transition-colors shadow-md flex items-center gap-2"
                >
                  {actionLoading === 'saving_display' ? (
                    <>
                      <FiRefreshCw className="animate-spin" />
                      <span>Saving...</span>
                    </>
                  ) : saveFeedback === 'display_saved' ? (
                    <>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-amber-50/80 border border-amber-200 p-6 rounded-2xl space-y-4">
              ...
              <p className="text-xs text-amber-900/80 leading-relaxed">
                Displays a scheduled maintenance notice banner at the top of all public pages and admin panel before maintenance begins.
              </p>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="church-label text-amber-950">From Date & Time (Start) *</label>
                  <input
                    type="datetime-local"
                ...
                {settings?.noticeBanner?.isEnabled ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    ...
                    >
                      ...
                      <span>{actionLoading === 'showcase_banner' ? 'Updating...' : 'Update Showcase'}</span>
                    </button>
                    ...
                    >
                      ...
                      <span>{actionLoading === 'stop_banner' ? 'Stopping...' : 'Stop Banner'}</span>
                    </button>
                  </div>
                ) : (
                  <button
                  ...
                  >
                    <FiSend className="text-sm" />
                    <span>{actionLoading === 'showcase_banner' ? 'Showcasing Notice Banner...' : 'Showcase Notice Banner'}</span>
                  </button>
                )}
              </div>
            </div>
      ...
      {/* Tab 2: Scheduler (Same Central State Operation) */}
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-6 max-w-3xl">
          <div className="border-b pb-4">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Schedule maintenance mode to start and end automatically using the same atomic state control service.
            </p>
          </div>
          ...
          <label className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 border border-gray-200 cursor-pointer">
            ...
            <div>
              ...
              <span className="text-xs text-gray-500">The system automatically engages MAINTENANCE and restores to LIVE at the specified timestamps.</span>
            </div>
          </label>
          ...
          <div className="pt-2 flex items-center gap-3">
            <button
              ...
              disabled={actionLoading !== null}
              className="px-6 py-2.5 bg-church-gold hover:bg-amber-600 text-white font-bold text-xs sm:text-sm rounded-xl cursor-pointer disabled:opacity-50 shadow-md flex items-center gap-2"
            >
              {actionLoading === 'saving_scheduler' ? (
                <>
                  <FiRefreshCw className="animate-spin" />
                  <span>Saving Schedule...</span>
                </>
              ) : saveFeedback === 'schedule_saved' ? (
                <>
                  <FiCheckCircle className="text-base" />
          ...
          <div className="flex items-center justify-between border-b pb-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Complete historical record of maintenance activations, emergency shutdowns, and multi-channel notifications.
              </p>
            </div>
        ...
        {enableModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-white rounded-2xl p-6 sm:p-8 max-w-md w-full border border-amber-200 shadow-2xl space-y-5"
            >
              <div className="flex items-center gap-3 text-amber-600">
                ...
                <div>
                  <h3 className="font-extrabold text-lg text-amber-950">Enable Maintenance Mode</h3>
                  <p className="text-xs text-amber-700 font-semibold">Immediate Operational Transition</p>
                </div>
              </div>

              <p className="text-xs text-gray-600 leading-relaxed">
                Activating maintenance mode will immediately switch public pages to the maintenance display and restrict normal WhatsApp bot services. Background multi-channel notifications will be dispatched.
              </p>

              <div>
                <label className="church-label">Maintenance Notice / Reason</label>
                <textarea
                  rows={2}
                  value={customReason}
                  onChange={(e) => setCustomReason(e.target.value)}
                  className="church-input border-amber-300 focus:border-amber-500"
                  placeholder="e.g. Scheduled System Upgrades and Enhancements"
                />
              </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <button
                  ...
                  className="w-1/2 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold transition-colors shadow-md cursor-pointer"
                >
                  Confirm & Enable
                </button>
              </div>
            ...
            >
              <div className="flex items-center gap-3 text-red-600">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-600 font-semibold">Immediate Destructive Lockdown</p>
                </div>
              </div>

              <p className="text-xs text-gray-600 leading-relaxed">
                This will immediately lock down the website for public visitors and members, restrict the WhatsApp bot, and log an emergency audit trail.
              </p>
